...
# Toggle script for Node.js demo
...
CURRENT_VERSION=$(grep "^APP_VERSION=" "$CONFIG_FILE" | cut -d'=' -f2)
...
sed -i.bak "s/^APP_VERSION=.*/APP_VERSION=$NEW_VERSION/" "$CONFIG_FILE"
sed -i.bak "s/^APP_DEPLOYMENT_COLOR=.*/APP_DEPLOYMENT_COLOR=$NEW_COLOR/" "$CONFIG_FILE"
